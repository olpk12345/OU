...
$('fileInput').addEventListener('change', async (event) => { const file = event.target.files[0]; if (!file) return; $('uploadButton').disabled = true; showNotice(`正在上传并解析 ${file.name}，文件较大时可能需要几分钟`); try { const filenameBytes = new TextEncoder().encode(file.name); let binaryName = ''; for (const byte of filenameBytes) binaryName += String.fromCharCode(byte); const filenameBase64 = btoa(binaryName); const preview = await fetch('/api/uploads/preview',{method:'POST',headers:{'X-Filename-Base64':filenameBase64,'Content-Type':file.type||'application/octet-stream'},body:await file.arrayBuffer()}); const data=await preview.json(); if(!preview.ok){showNotice(data.message||'清单解析失败');return} if(!window.confirm(`已识别新增 ${data.counts.inserted} 笔、更新 ${data.counts.updated} 笔，确认合并并计算？`)){return} const committed=await fetch(`/api/uploads/${data.batchId}/commit`,{method:'POST'}); if(!committed.ok){showNotice('清单合并失败');return} showNotice('清单已更新'); await loadPeriods(); await loadOverview(); } catch (error) { showNotice(`清单处理失败：${error.message}`); } finally { $('uploadButton').disabled = false; event.target.value=''; } });
